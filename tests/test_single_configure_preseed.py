...
# ---------------------------------------------------------------------------
# THE CLASS of defect behind two testbed findings (2026-09-25): `configure`
# writes only what it asks, so a value `.env.example` sets that the CODE's
# default does not match is silently lost on every configure-born install —
# CC_LLM_BASE_URL (the API could resolve no model) and CC_DEFAULT_MODEL (the
# first live triage was a 403 on an Anthropic model id). Every such key must be
# ASKED (a questions.tsv row), DERIVED by the app phase (`set_kv_if_unset`),
# or GENERATED by make-secrets.sh — or named here with the reason it is none.
# ---------------------------------------------------------------------------

# .env.example values that differ from the code default ON PURPOSE and are
# NOT the single-node profile's to write: <key>: <why>.
_EXAMPLE_ONLY_KEYS = {
    "CC_MCP_BUILD_HOST": "k3s profile: the node that builds MCP images; the single profile builds locally",
}
...
def test_every_example_value_the_code_does_not_default_is_asked_derived_or_generated():
    from central_command import config as cfg
    ...
    for key, value in sorted(example.items()):
        fname = key[3:].lower() if key.startswith("CC_") else None
        if fname not in fields:
            continue
        default = fields[fname].default
        if _same("" if default is None else str(default), value):
            continue
        if key in _EXAMPLE_ONLY_KEYS or key in asked:
            continue
        if re.search(rf'set_kv_if_unset "\$ENV_FILE" {key}\b', setup_src) or key in secrets_src:
            continue
        lost.append(f"{key}: .env.example={value!r} code default={default!r}")
    assert not lost, (
        "a configure-born .env silently loses these (ask them in questions.tsv, derive them "
        "in the app phase with set_kv_if_unset, or name them in _EXAMPLE_ONLY_KEYS with a reason):\n  "
        + "\n  ".join(lost)
    )
