...
`import` prints one line about `docs/vendor/` — 47k of the repo's 48k files,
which must travel in the zip because they are the air-gapped box's only offline
reference. The subtree carries a fingerprint (`docs/vendor/MANIFEST`,
regenerated by `scripts/vendor_manifest.sh`), and when the zip's fingerprint
equals the deployed one the import skips the whole subtree: `PASS vendor-docs:
