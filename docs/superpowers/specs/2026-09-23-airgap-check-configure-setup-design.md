...
- ~~`update.sh import` (Windows testbed, 2026-09-24): the unzip + tar-pipe tree
  ...
  four minutes.~~ **CLOSED (F19/F20, 2026-09-25):** not a git-native checkout in
  the end — a FINGERPRINT. `docs/vendor/` is 47,225 of the repo's 48,420 tracked
  files and it must stay in the zip (the air-gapped box's only offline
  reference), so the subtree carries `docs/vendor/MANIFEST` (a sha256 over the
  blob hash of every tracked path under it, `scripts/vendor_manifest.sh`,
  regenerated by both fetch scripts and guarded by
  `tests/test_vendor_manifest.py`) and `cmd_import` skips unzipping, `git rm`ing
  and tar-copying it whenever the zip's fingerprint equals the deployed one.
  Equal manifests mean identical trees, and a stale manifest cannot be released.
  `unzip`'s stderr is now a FAIL even at exit 0 (F20).
- F40 (2026-09-24): the suite hung forever when the graph's embedder did not
  answer. Closed in code — `graph_live` marker + a 10 s session preflight +
  `timeout = 120` in `pyproject.toml` — but its WINDOWS verification stays owed
  to the next testbed run: what has been measured here is the skip path with the
  backend up and down on Linux, not Git Bash's behaviour under pytest-timeout.
- F6 (2026-09-24): `check`'s memory probe read the HOST's RAM on a
  machine-backed podman. Closed with `cc_memory_verdict` in `machine-lib.sh`
  (judging `podman machine inspect --format '{{.Resources.Memory}}'`, MiB) —
  unit-tested with stubbed numbers, and UNVERIFIED against a real podman
  machine: there is none on the developer's Linux box. Next testbed run.
