...
what shipped**, checked against the code on 2026-09-20 — a spec's own header
says what was true when it was written, which for the withdrawn ones is no
longer the same thing.

| date | record | status |
|---|---|---|
| 09-03 | **Deploy refactor — the single-node install runs on Compose** — [spec](superpowers/specs/2026-09-03-deploy-refactor-design.md) *(added 2026-09-20)* | ◐ **D1+D2 shipped as v2.22.0** (2026-09-03: one `compose.yaml` replaces the bespoke choreography; image versions gained a constraint/lock/resolve seam) — **D3 (update pipeline) partial**: `deploy/single/update.sh` has plan/apply/verify/rollback, but not the specced separate stage/pre-test phases or plan-time constraint resolution against the mirror; **D4 (k3s Helm chart) not started** — `deploy/k3s/` still runs the bespoke `cc-update.sh`/manifest choreography this record's phase 3 was meant to replace |
...
| 08-09 | **Graph inspection & curation (the rung ladder)** — [spec](superpowers/specs/2026-08-09-graph-inspection-design.md) | ◐ **rungs 1–3 built with the spec** (browse recipe, `graph_inspect.py`, cockpit Graph panel + Graphistry three-state seam); **curation writes built 2026-08-15, but not as the spec's "next increment" originally proposed (corrected 2026-09-20)** — not a proposal through the Executor, but a closed write set in `integrations/neo4j_writer.py` the API tier calls directly (the operator reviewing his OWN click is ceremony, not governance); Graphistry integration remains the one deferred increment |
...
| 07-26 | **Adaptive routing that learns** — [spec](superpowers/specs/2026-07-26-adaptive-routing-that-learns-design.md) · [plan (Units 1–2)](superpowers/plans/2026-07-26-adaptive-routing-units-1-2.md) | ◐ **partly landed** — Units 1–2 (declared tiers, costs, fallback DECLARATIONS in `deploy/pi/litellm/model-preferences.yaml`) are applied, but no fallback is actually configured: `fallbacks: {}` there, by operator decision 2026-07-31 — a workstation outage was silently reaching for Anthropic and dying on a token-limit mismatch anyway, so failing fast with the real `Connection error` replaced it **(corrected 2026-09-20)**; Unit 3 (the feedback loop) was withdrawn by the quality-router spec |
| 07-26 | **Local model serving + embedding migration** — [spec](superpowers/specs/2026-07-26-local-serving-and-embedding-migration-design.md) | ◐ **Units 2–4 built (corrected 2026-09-20)** — the co-resident embedder, Graphiti repointed through LiteLLM and the stamped, resumable re-embed (`integrations/neo4j_writer.py`, `scripts/oneoff/reembed_graph.py`) are live; Unit 1 (llama-server router mode) was not built as specified — the instance runs `llama-swap` instead, a different serving mechanism for the same one-model-at-a-time goal |
