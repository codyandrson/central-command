...
| DL-005 | The agent sandbox is containment, not restriction | active | undated | discipline only | [trust-gate.md](trust-gate.md) |
...
| DL-009 | A proposal's agent_id is the drafter, never the subject | active | undated | discipline only | [trust-gate.md](trust-gate.md) |
...
| DL-017 | Roster membership is a non-empty role column; hire it, don't upsert it | active | undated | test: `tests/test_hiring.py::test_ensure_hired_creates_a_full_roster_member_and_is_idempotent` | [trust-gate.md](trust-gate.md) |
...
| DL-021 | Two run modes: CC_DEMO_MODE and CC_EXECUTOR_MODE | active | undated | code structure only | [runtime.md](runtime.md) |
| DL-022 | A fresh run must load the charter; build_agent_for does not | active | undated | discipline only | [runtime.md](runtime.md) |
...
| DL-024 | Agents take deps | active | undated | discipline only | [runtime.md](runtime.md) |
| DL-025 | Sessions are never destroyed; agent:<id>:main means the current lane only | active | undated | code structure only | [runtime.md](runtime.md) |
...
| DL-028 | A failure-landing guard that lives in one wrapper is not a landing | active | undated | discipline only | [runtime.md](runtime.md) |
| DL-029 | A shutdown hook in the FastAPI lifespan runs too late; release from SIGTERM instead | active | undated | discipline only | [runtime.md](runtime.md) |
...
| DL-041 | A Jira gadget's config keys are declared by the gadget, read from its XML | active | undated | code structure only | [ingest-integrations.md](ingest-integrations.md) |
| DL-042 | n8n import:workflow never activates outside queue mode; the canvas is not the source of truth | active | undated | script: `deploy/n8n/apply-workflows.sh` | [ingest-integrations.md](ingest-integrations.md) |
...
| DL-045 | An unsubscribe URL is derived from the mailbox, never from the proposal | active | undated | code structure only | [ingest-integrations.md](ingest-integrations.md) |
| DL-046 | Ledger invariants live in SQL, not Python, and are never mocked in tests | active | undated | test: `tests/test_ledger.py` | [ingest-integrations.md](ingest-integrations.md) |
| DL-047 | Header-less email still needs a stable key, synthesised from the content hash | active | undated | test: `tests/test_ledger.py::test_bare_text_gets_a_stable_synthetic_message_id` | [ingest-integrations.md](ingest-integrations.md) |
| DL-048 | Dispatch is opt-in; the drain loop never starts by surprise | active | undated | code structure only | [ingest-integrations.md](ingest-integrations.md) |
...
| DL-050 | Every email takes one path: dispatcher.process_claimed() | active | undated | code structure only | [ingest-integrations.md](ingest-integrations.md) |
| DL-051 | A Graphiti entity and edge each store their identity twice | active | undated | discipline only | [graph.md](graph.md) |
| DL-052 | Every write re-embeds at exactly 1024 dimensions; a mis-sized vector is dropped | active | undated | discipline only | [graph.md](graph.md) |
...
| DL-058 | Episodes name the operator; "the operator" is not a graph subject | active | undated | test: `tests/test_context_overflow.py::test_graph_propose_notes_tell_the_agent_to_name_the_operator` | [graph.md](graph.md) |
| DL-059 | An edge with expired_at is not necessarily a retired fact | active | 2026-09-15 | discipline only | [graph.md](graph.md) |
| DL-060 | The ontology needs somewhere for every category; declare high-priority types first | active | undated | discipline only | [graph.md](graph.md) |
...
| DL-066 | Autodiscovery has memory; only new/changed/failed ids reach the agent | active | undated | UNVERIFIED | [models.md](models.md) |
...
| DL-080 | Use the fully-qualified image ref; podman's localhost/ tag is invisible to Kubernetes | active | undated | script: `grep -qx` | [deploy.md](deploy.md) |
...
| DL-086 | A deleted manifest takes its resource with it, or it should — kubectl apply does not prune | active | 2026-09-01 | discipline only | [deploy.md](deploy.md) |
| DL-087 | @container must sit on a parent of whatever uses @3xl: | active | undated | test: `web/src/features/container-query-scope.test.ts` | [cockpit.md](cockpit.md) |
...
| DL-090 | _COMPOSER_DISABLING is an allowlist, not a check; a new refusal code needs its own guard | active | undated | code structure only | [cockpit.md](cockpit.md) |
