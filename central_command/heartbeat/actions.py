...
def _add_brief(credential_name: str, models: list[dict]) -> str:
    ...
    return (
        ...
        "registered anywhere, under any alias) — research each one (context "
        "limits, vision/tool support — from the catalog metadata below, plus "
        "a web read if useful), then make ONE batched proposal for EXACTLY "
        "these models via propose_litellm_change, one litellm.add_model "
        "action per model. Every add MUST set "
