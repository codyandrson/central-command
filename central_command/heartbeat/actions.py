...
def _add_brief(credential_name: str, models: list[dict]) -> str:
    ...
    return (
        ...
        "counterpart if one exists, for cost-map defaults). Once approved, "
        "run litellm_probe_model on each and propose litellm.update_model "
        "with its `suggested_model_info` — that is where supports_* and the "
        "token limits come from, never from a guess. Every add MUST set "
