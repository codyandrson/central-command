...
## Goal Preview Markers — `[goal-preview:{...}]`

Shows a draft goal (line of effort) tile inline while the operator and agent are still discussing it — before any `loe.create` proposal is drafted.

### Format

```
[goal-preview:{"name":"<name>","cadence":"<cadence>","questions":["<question>", ...],"thresholds":"<thresholds>"}]
```

`name` is required and must be a non-empty string. `cadence`, `questions`, and `thresholds` are all optional.
...
[goal-preview:{"name":"Ship v2 onboarding","cadence":"weekly","questions":["Is the checklist completing end to end?"],"thresholds":"green if >80% of new users finish it"}]

Let me know if the cadence should be different.
```
...
- The marker text is stripped from the visible message — add context before/after
- This is a PREVIEW only — it renders no check-in data and does not create anything; use `propose_loe`'s `loe.create` capability (once approved) to actually create the goal

### Implementation

- **Parser**: `src/features/loe/extractGoalPreviews.ts` — bracket-balanced `[goal-preview:{...}]` extraction with JSON validation, cloned from `extractCharts.ts`
- **Renderer**: `src/features/loe/GoalPreviewCard.tsx`
- **Decisions Inbox**: `src/features/decisions/DecisionsView.tsx`'s `loe.create` case renders the same card over the proposal's action arguments
