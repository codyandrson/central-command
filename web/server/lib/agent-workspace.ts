...
/**
 * True when the resolved workspace for `agentId` is the *computed default*
 * path — i.e. there is no explicit `openclaw.json` entry for it (and it
 * isn't `main`, whose root comes from `config.memoryPath`). Safe to
 * auto-create if missing; an explicitly-configured-but-missing path is a
 * real misconfiguration that should stay visible instead.
 */
export function isDefaultAgentWorkspacePath(agentId?: string): boolean {
  const normalizedAgentId = normalizeAgentId(agentId);
  if (normalizedAgentId === 'main') return false;
  return !getConfiguredAgentWorkspace(normalizedAgentId);
}
