...
// Central Command's gateway is the FastAPI app under uvicorn (cc-uvicorn.service),
// not Nerve's `openclaw-gateway` binary — match the module on the cmdline.
const GATEWAY_CMDLINE = 'central_command.api.app';

async function getGatewayPidFromPgrep(): Promise<string> {
  const stdout = await execFileText('pgrep', ['-f', GATEWAY_CMDLINE]);
  return stdout.split('\n')[0] || '';
}

async function getGatewayPidFromPs(): Promise<string> {
  const stdout = await execFileText('ps', ['-axo', 'pid=,args=']);
  for (const line of stdout.split('\n')) {
    const match = line.trim().match(/^(\d+)\s+(.+)$/);
    if (!match) continue;
    const [, pid, args] = match;
    if (args.includes(GATEWAY_CMDLINE)) return pid;
  }
  return '';
}
