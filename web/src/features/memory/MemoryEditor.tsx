...
export function MemoryEditor({ agentId, title, date, onSave, onCancel }: MemoryEditorProps) {
    ...
    const fetchContent = async () => {
      try {
        ...
        const data = await apiFetch<{ content?: string }>(
          `/api/memories/section?${params.toString()}`,
  ...
  const handleSave = async () => {
    ...
    try {
      await apiFetch('/api/memories/section', {
        method: 'PUT',
    ...
    <div className="h-full flex flex-col min-h-0">
      ...
      <div className="panel-header border-l-[3px] border-l-purple">
        <button
          ...
          className="text-muted-foreground hover:text-foreground transition-colors p-1.5 -ml-2"
          title="Back to list"
        >
          <ArrowLeft size={14} />
        </button>
