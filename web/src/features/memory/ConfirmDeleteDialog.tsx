  ...
  useEffect(() => {
    ...
    const fetchContent = async () => {
      ...
      try {
        ...
        const data = await apiFetch<{ content?: string }>(`/api/memories/section?${params.toString()}`);
        setMarkdownContent(data.content || null);
      } catch {
        setMarkdownContent(null);
      } finally {
          ...
          {(isSection || isDaily) && (
            <div className="max-h-60 overflow-y-auto rounded-2xl border border-destructive/20 bg-destructive/6 px-3 py-3">
              ...
              {contentLoading ? (
                ...
                <pre className="whitespace-pre-wrap cockpit-wrap break-words font-mono text-[0.733rem] text-muted-foreground">
                  {markdownContent}
                </pre>
              ) : hasItems ? (
