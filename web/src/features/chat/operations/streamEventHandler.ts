...
export function extractStreamDelta(
  chatPayload: ChatEventPayload,
): { text: string; cleaned: string; ttsText: string | null; charts: ChartData[]; goalPreviews: GoalPreviewData[] } | null {
  ...
  const { cleaned, goalPreviews } = extractGoalPreviews(chartCleaned);
  return { text: deltaText, cleaned, ttsText, charts, goalPreviews };
}
...
export function extractFinalMessage(chatPayload: ChatEventPayload): FinalMessageData | null {
  ...
  const { cleaned: chartCleaned, charts } = extractChartMarkers(text);
  const { goalPreviews } = extractGoalPreviews(chartCleaned);
  return { message: representative, text, ttsText, charts, goalPreviews };
}
