...
describe('groupToolMessages', () => {
  ...
  it('pairs a toolResult onto its call by matching toolCallId, dropping its own row', () => {
    const msgs: ChatMsg[] = [
      { role: 'tool', html: '', rawText: '**tool:** `read`\n```json\n{}\n```', timestamp: new Date(), toolCallId: 'call_1', toolName: 'read' },
      { role: 'tool', html: '', rawText: '**tool:** `write`\n```json\n{}\n```', timestamp: new Date(), toolCallId: 'call_2', toolName: 'write' },
      { role: 'toolResult', html: '', rawText: 'file contents here', timestamp: new Date(), toolCallId: 'call_1' },
    ];
    const result = groupToolMessages(msgs);
    expect(result).toHaveLength(1);
    // Only the two calls remain as entries — the result was paired, not appended as a third row.
    expect(result[0].toolGroup).toHaveLength(2);
    expect(result[0].toolGroup?.[0].resultText).toBe('file contents here');
    expect(result[0].toolGroup?.[1].resultText).toBeUndefined();
  });

  it('keeps an unmatched toolResult as its own row (no toolCallId, or no matching call)', () => {
    const msgs: ChatMsg[] = [
      { role: 'tool', html: '', rawText: '**tool:** `read`\n```json\n{}\n```', timestamp: new Date(), toolCallId: 'call_1', toolName: 'read' },
      { role: 'toolResult', html: '', rawText: 'orphaned result, no matching call', timestamp: new Date() },
    ];
    const result = groupToolMessages(msgs);
    expect(result).toHaveLength(1);
    expect(result[0].toolGroup).toHaveLength(2);
    expect(result[0].toolGroup?.[0].resultText).toBeUndefined();
    expect(result[0].toolGroup?.[1].rawText).toBe('orphaned result, no matching call');
  });
});
