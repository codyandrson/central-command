...
import type { ChatMessage, ContentBlock, ChatHistoryResponse, ChatComposerState, SourceEmail } from '@/types';
...
import { extractGoalPreviews } from '@/features/loe/extractGoalPreviews';
...
export function splitToolCallMessage(m: ChatMessage, options: { sessionKey?: string } = {}): ChatMsg[] {
  ...
  if (m.role === 'assistant' && Array.isArray(m.content)) {
    ...
    if (hasTools || hasThinking) {
      ...
      const flushText = () => {
        ...
        const { cleaned: goalPreviewCleaned, goalPreviews } = extractGoalPreviews(chartCleaned);
        const { cleaned, images: extractedImages } = extractImages(goalPreviewCleaned);
        if (cleaned.trim() || extractedImages.length > 0) {
          result.push({
            ...
            ...(goalPreviews.length > 0 ? { goalPreviews } : {}),
    ...
    if (segments.some(s => s.role === 'event')) {
      return segments.map(seg => {
        ...
        return {
          ...
          html: renderToolResults(renderMarkdown(goalPreviewCleaned)),
          rawText: goalPreviewCleaned,
          timestamp,
          streaming: false,
          ...(charts.length > 0 ? { charts } : {}),
          ...(goalPreviews.length > 0 ? { goalPreviews } : {}),
  ...
  const { cleaned: text, images: extractedImages } = isAssistant
    ? extractImages(goalPreviewCleaned)
    : { cleaned: goalPreviewCleaned, images: [] };
  ...
  return [{
    ...
    ...(m.role === 'toolResult' && m.toolName ? { toolName: m.toolName } : {}),
  }];
}
...
export function groupToolMessages(msgs: ChatMsg[]): ChatMsg[] {
  ...
  const flushTools = () => {
    ...
    // Raw results render too now (collapsed one-line preview, full body on expand) — only
    // image-only results get pulled out, since ToolCallBlock has nowhere to show an image.
    const filtered: ChatMsg[] = [];
    // Pair each toolResult onto the call preceding it that shares its toolCallId
    // (matched by ID, not position — a call can be followed by other calls
    // before its own return lands). Unmatched results (no id, or a history
    // written before the backend carried one) keep their own row.
    const callIndexByToolCallId = new Map<string, number>();
    for (const [i, t] of toolBuffer.entries()) {
      if (t.rawText.startsWith('**tool:**') && t.toolCallId) {
        callIndexByToolCallId.set(t.toolCallId, i);
      }
    }
    const consumedResultIndexes = new Set<number>();
    for (const [i, t] of toolBuffer.entries()) {
      const isToolUse = t.rawText.startsWith('**tool:**');
      if (isToolUse || !t.toolCallId) continue;
      const callIdx = callIndexByToolCallId.get(t.toolCallId);
      if (callIdx !== undefined) {
        toolBuffer[callIdx] = { ...toolBuffer[callIdx], resultText: t.rawText };
        consumedResultIndexes.add(i);
      }
    }
    ...
    for (const [i, t] of toolBuffer.entries()) {
      ...
      const isToolUse = t.rawText.startsWith('**tool:**');
      const hasEdits = !isToolUse && extractEditBlocks(t.rawText).length > 0;
      const hasWrites = !isToolUse && extractWriteBlocks(t.rawText).length > 0;
      if (isToolUse || hasEdits || hasWrites || t.rawText.trim()) {
        filtered.push(t);
    ...
    if (filtered.length === 1) {
      ...
      const entries: ToolGroupEntry[] = filtered.map(t => {
        ...
        return {
          html: t.html,
          rawText: t.rawText,
          preview: plainPreview || t.rawText.slice(0, 80),
          toolName: t.toolName,
...
export async function loadChatHistory(params: {
  ...
  limit?: number;
  /**
   * Central Command: the same payload carries whether this session accepts input
   * (see ChatComposerState). Delivered as a callback so every existing caller
   * keeps its ChatMsg[] return; the callers that own the composer opt in.
   */
  onComposer?: (composer: ChatComposerState | undefined, sessionKey: string) => void;
  /** Central Command: the email this lane was spawned to handle, same instant as
   *  the transcript — so the header chip never shows a different session's
   *  email than the one on screen. */
  onSourceEmail?: (sourceEmail: SourceEmail | null | undefined, sessionKey: string) => void;
}): Promise<ChatMsg[]> {
  const { rpc, sessionKey, limit = 100, onComposer, onSourceEmail } = params;

  const res = await rpc('chat.history', { sessionKey, limit }) as ChatHistoryResponse;
  const msgs = res?.messages || [];
  onComposer?.(res?.composer, sessionKey);
  onSourceEmail?.(res?.sourceEmail, sessionKey);
