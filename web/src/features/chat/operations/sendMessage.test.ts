...
describe('sendChatMessage', () => {
  ...
  it('sends the filename, which the gateway needs to pick a converter', async () => {
    // Not decoration: markitdown selects its converter from the extension, so a
    // dropped name makes every attachment an extensionless blob. This assertion
    // is the frontend half of central_command/api/attachments.py's contract.
    const rpc = vi.fn().mockResolvedValue({});
    ...
    await sendChatMessage({
      ...
      text: 'have a look',
      images: [
        { id: '1', mimeType: 'application/pdf', content: 'b64', preview: '', name: 'contract.pdf' },
      ],
      idempotencyKey: 'k1',
    });

    expect(rpc.mock.calls[0][1].attachments[0].name).toBe('contract.pdf');
  });
