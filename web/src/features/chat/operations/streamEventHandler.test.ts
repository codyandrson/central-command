...
describe('buildActivityLogEntry', () => {
  ...
  it('describes a Central Command tool with its identifying argument', () => {
    const entry = buildActivityLogEntry({
      stream: 'tool',
      data: { phase: 'start', name: 'jira_search', toolCallId: 'tc-2',
              args: { jql: 'project = T1' } },
    });
    expect(entry!.description).toBe('jira_search: project = T1');
  });
