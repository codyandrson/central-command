...
export async function sendChatMessage(params: {
  ...
  if (images?.length) {
    // `name` is load-bearing, not decoration: the gateway hands the filename to
    // markitdown, whose converter selection starts from the extension. Dropping
    // it (as this did until 2026-08-11) makes every attachment an extensionless
    // blob and leans the whole extraction on content sniffing alone.
