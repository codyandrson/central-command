...
import { useGatewayModelCatalog, type GatewayModelInfo } from '@/hooks/useGatewayModelCatalog';
...
// Exactly the levels the serving model's chat template accepts (the gateway
// validates against the same set — central_command/runtime/models.py THINKING_LEVELS).
type EffortLevel = 'off' | 'low' | 'medium' | 'xhigh';
type EffortSelection = typeof INHERITED_EFFORT_VALUE | EffortLevel;
const EFFORT_OPTIONS: EffortLevel[] = ['off', 'low', 'medium', 'xhigh'];
...
/**
 * The effort levels selectable for a given catalog entry.
 *
 * `thinkingLevels` absent (older payload, or a model not yet in the catalog)
 * means "unknown" — offer the full set rather than guessing it has none.
 * `thinkingLevels: []` is an explicit "no thinking control" from the gateway
 * and is honored as such: the effort control disables for that model.
 */
function effortLevelsFor(entry: GatewayModelInfo | undefined): EffortLevel[] {
  if (!entry || entry.thinkingLevels === undefined) return EFFORT_OPTIONS;
...
export interface UseModelEffortReturn {
  ...
  /** True when the selected model declares no thinking levels — the effort
   *  control has nothing to offer and should read as disabled, not empty. */
...
export function useModelEffort(): UseModelEffortReturn {
  ...
  const { models: gatewayModels, error: catalogError } = useGatewayModelCatalog();
  ...
  // Effort options follow the SELECTED model, not a fixed list — a model with
  // no declared thinking levels has nothing for the effort control to offer.
  const selectedModelEntry = useMemo(() => {
    const resolvedId = selectedModel === INHERITED_MODEL_VALUE ? primaryModelId : selectedModel;
    return modelOptionsList.find((m) => m.id === resolvedId);
  }, [selectedModel, primaryModelId, modelOptionsList]);
  const availableEffortLevels = useMemo(
    () => effortLevelsFor(selectedModelEntry),
    [selectedModelEntry],
  );
  ...
  // Surface the shared catalog's load error (once it has resolved either way).
  useEffect(() => {
    if (gatewayModels === null) return;
    setUiError(buildModelCatalogUiError(gatewayModels, catalogError));
  }, [gatewayModels, catalogError]);
  ...
  // 3. If all WS attempts fail, surface the error — there is no HTTP fallback
    ...
    try {
      ...
      try {
        ...
        if (!wsSucceeded) {
          ...
          console.warn('[useModelEffort] sessions.patch failed:', wsError);
        }
      }

      // There is no HTTP fallback: /api/gateway/session-patch posted to an
      // OpenClaw gateway Central Command does not run, so it answered 405 every
      // time and only ever converted a WS failure into a misleading error.
      // sessions.patch over the WS is the real write path.
      if (!wsSucceeded) {
        throw new Error(`Model change failed: ${wsError || 'the session RPC did not apply it'}`);
      }
      ...
      // The new model may not support the effort level currently selected —
      // clear it rather than silently keep an override the new model can't
      // honor (e.g. an explicit "xhigh" carried over onto a model with no
      // thinking control at all).
      const resolvedNextId = selectingInheritedPrimary ? primaryModelId : (patchModel || next);
      const nextLevels = effortLevelsFor(modelOptionsList.find((m) => m.id === resolvedNextId));
      if (selectedEffort !== INHERITED_EFFORT_VALUE && !nextLevels.includes(selectedEffort as EffortLevel)) {
        await handleEffortChange(INHERITED_EFFORT_VALUE);
      }

      // Release the model lock once the optimistic value has had time to settle.
      // This used to re-read the model back from /api/gateway/session-info to
      // confirm it; that route only ever called an OpenClaw gateway Central Command
      // does not run, so the confirmation never once ran — the optimistic value
      // was always what stood.
  ...
  }, [controlsDisabled, selectedModel, selectedEffort, rpc, currentSession, updateSession, modelOptionsList, primaryModelId, model, handleEffortChange]);

  // The inherited option is a sentinel ("no override" → sessions.patch model:null),
  // but it must NAME the concrete model the agent sends to LiteLLM — labelling it
  // "primary" made the dropdown read as a role instead of a model (operator report).
  const modelOptions = useMemo(() => {
    const configured = modelOptionsList.map((m) => ({ value: m.id, label: m.label }));
    const primaryLabel = primaryModelId
      ? (modelOptionsList.find((m) => m.id === primaryModelId)?.label || primaryModelId)
      : null;
    return [
      { value: INHERITED_MODEL_VALUE, label: primaryLabel ? `Default (${primaryLabel})` : 'Default' },
      ...configured,
    ];
  }, [modelOptionsList, primaryModelId]);
  ...
  const effortOptions = useMemo(
    () => [
      ...
      ...availableEffortLevels.map((lvl) => ({ value: lvl, label: lvl })),
    ],
    [inheritedEffortLabel, availableEffortLevels],
  );
  ...
  return {
    ...
    handleEffortChange,
    controlsDisabled,
    effortDisabled: controlsDisabled || availableEffortLevels.length === 0,
    uiError,
  };
}
