...
function ToolEntryRow({ entry }: { entry: ToolGroupEntry }) {
  ...
  const hasEditsOrWrites = editBlocks.length > 0 || writeBlocks.length > 0;

  // Every entry is expandable now: edit/write render as diff/file views,
  // everything else shows its args (and, if paired, its result) as raw text.
  return (
      ...
      <CollapsibleContent>
        <div className="ml-6 mr-2 mb-2 mt-1 border-l-2 border-border/40 pl-3">
          {hasEditsOrWrites ? (
            ...
            <div className="space-y-2">
              <pre className="msg-body whitespace-pre-wrap break-words text-[0.8rem] font-mono opacity-85 max-h-[300px] overflow-y-auto">
                {entry.rawText}
              </pre>
              {entry.resultText ? (
                <div>
                  <div className="text-[0.667rem] uppercase tracking-wide text-muted-foreground mb-1">result</div>
                  <pre className="msg-body whitespace-pre-wrap break-words text-[0.8rem] font-mono opacity-85 max-h-[300px] overflow-y-auto">
                    {entry.resultText}
                  </pre>
                </div>
