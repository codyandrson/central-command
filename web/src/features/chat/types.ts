...
export interface ToolGroupEntry {
  ...
  preview: string;
  /** Tool name, when the entry originated from a tool_use block */
  toolName?: string;
  /** The paired tool-return content, matched by toolCallId */
  resultText?: string;
}
...
export interface ChatMsg {
  ...
  charts?: ChartData[];
  /** Extracted [goal-preview:{...}] markers for inline draft-goal rendering */
  ...
  systemLabel?: string;
  /** Tool name — set on 'tool' (call) and 'toolResult' rows */
  toolName?: string;
  /** Identifies the tool call this row belongs to (call or its result) */
  toolCallId?: string;
  /** The raw args object for a tool_use row (pretty-printed in rawText already) */
  toolArgs?: Record<string, unknown>;
  /** The paired tool-return content, matched onto the call by toolCallId */
  resultText?: string;
}
