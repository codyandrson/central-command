...
import { ActivityLog, ChatHeader, ProcessingIndicator, ReadOnlySessionNotice, ScrollToBottomButton, StreamingMessage, ToolGroupBlock } from './components';
import { isMessageCollapsible } from './types';
import type { ChatMsg, ImageAttachment, OutgoingUploadPayload } from './types';
import type { ChatComposerState, SourceEmail } from '@/types';
...
interface ChatPanelProps {
  ...
  /** Central Command: whether this session takes operator input, and if not, why.
   *  Served with the history, so what the operator sees and what a send does
   *  come from one decision on the server. */
  ...
  /** Open the Decisions Inbox, optionally focused on the blocking ask. */
...
export const ChatPanel = forwardRef<ChatPanelHandle, ChatPanelProps>(function ChatPanel({
  ...
  // Whether this transcript has ended, and which ending to name — one rule,
  // in sessionEnd.ts, so visibility and text cannot disagree.
  ...
  return (
    <div id={id} className="h-full flex flex-col border-r border-border min-w-0 relative">
      ...
      >
        ...
        {/* End of a closed conversation — the last thing in the transcript. */}
        {ending && (
          <div
            role="separator"
            data-testid="session-terminal-divider"
            className="flex items-center gap-2 px-4 py-3 text-muted-foreground/60 text-[0.667rem] tracking-widest uppercase select-none"
          >
            <span className="h-px flex-1 bg-border/60" aria-hidden="true" />
            <span>{ENDING_LABEL[ending]}</span>
            <span className="h-px flex-1 bg-border/60" aria-hidden="true" />
          </div>
        )}
      ...
      {/* Input area — or, for a session that does not take messages (a paused
          task run, a conversation holding an undecided proposal), the reason
          and where to act instead. Never an input the server would refuse. */}
      {composer && composer.enabled === false ? (
