...
describe('InputBar', () => {
  ...
  it('does not open the slash command menu — the list is trimmed to empty (no server-side commands wired)', async () => {
    render(<InputBar onSend={vi.fn()} isGenerating={false} />);
  ...
  it('offers the paperclip without the vendored attachment menu', async () => {
    // Attachments are delivered now (documents are extracted to text at the
    // gateway; anything undeliverable is refused with a reason), so the
    // under-construction disable is gone. The vendored multi-action menu and
    // browse-by-path affordances stay absent — that half is still not wired.
    render(<InputBar onSend={vi.fn()} isGenerating={false} />);

    const attachButton = await screen.findByLabelText('Attach a document');
  ...
  it('keeps the draft and the staged file when the gateway refuses the send', async () => {
    // The gateway refuses an attachment it cannot deliver (an image no model
    // can read, a scan that extracts to nothing) so the operator can FIX it —
    // swap the file, change the model, drop it, or reword the message. That
    // only works if their message is still there. The input is cleared
    // optimistically before the await, so this is the restore path.
    const onSend = vi.fn().mockRejectedValue(
      new Error("'contract.pdf' converted but contains no readable text — it is most likely a scan"),
    );
    render(<InputBar onSend={onSend} isGenerating={false} />);

    const textarea = screen.getByLabelText('Message input') as HTMLTextAreaElement;
    fireEvent.input(textarea, { target: { value: 'summarise the attached contract' } });

    const fileInput = document.querySelector('input[type="file"]') as HTMLInputElement;
    const doc = new File([new Uint8Array(2048)], 'contract.pdf', { type: 'application/pdf' });
    ...
    expect(textarea.value).toBe('summarise the attached contract');
    // …and so did the file, so "remove it and re-send" is one click.
    expect(screen.getByText('contract.pdf')).toBeInTheDocument();
  });
