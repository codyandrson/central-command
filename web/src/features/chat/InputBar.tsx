...
// None of the vendored Nerve slash commands are wired server-side in
// Central Command (verified 2026-08-03: no route/handler anywhere in
// web/server or central_command/api intercepts a leading "/" — the typed text
// goes straight to the LLM as a chat message). Trimmed the decorative list
// to empty rather than delete the autocomplete machinery below, so a real
// command can be added here the day one is actually wired up.
...
export const InputBar = forwardRef<InputBarHandle, InputBarProps>(function InputBar({
  ...
  const typingResizeFrameRef = useRef<number | null>(null);
  ...
  // Coalesces rapid `input` events (fast typing, paste) into at most one
  // resize/layout-read per animation frame, same rAF idiom as
  // scheduleDeferredResize below but single-shot (no settle pass needed —
  // handleInput isn't injecting a selection change).
  const scheduleResize = useCallback(() => {
    if (typingResizeFrameRef.current !== null) return;
    typingResizeFrameRef.current = requestAnimationFrame(() => {
      typingResizeFrameRef.current = null;
      resizeInput();
    });
  }, [resizeInput]);
  ...
  useEffect(() => () => {
    ...
    if (typingResizeFrameRef.current !== null) {
      cancelAnimationFrame(typingResizeFrameRef.current);
    }
  }, []);

  useEffect(() => {
    persistedComposerSnapshot = {
      // The DOM is the source of truth for an uncontrolled textarea; draftText
      // only tracks programmatic changes (handleInput skips it on purpose).
      text: inputRef.current ? inputRef.current.value : draftText,
      stagedAttachments,
      attachmentError,
  ...
  const handleSend = useCallback(async () => {
    ...
    try {
      ...
      // PUT THE DRAFT BACK. The input is cleared optimistically above (before
      // the await) so typing feels instant, which means a REFUSED send would
      // otherwise destroy what the operator wrote — and an attachment refusal
      // is precisely the case where they need their text: the whole point of
      // refusing at the gateway is that they can fix the file, swap the model,
      // or drop the attachment and re-send the SAME message. Staged files are
      // already kept (clearStagedAttachments only runs on success).
      const input = inputRef.current;
      if (input && text) {
        input.value = text;
        input.style.height = 'auto';
  ...
  const handleInput = () => {
    if (!inputRef.current) return;
    const input = inputRef.current;
    // Deliberately NOT setDraftText here: the textarea is uncontrolled, so a
    // keystroke needs no render — draftText state is only for programmatic
    // changes (inject/history/voice/send). Persist the draft directly so a
    // remount still restores it. Keeps plain typing at zero React renders and
    // one resizeInput reflow instead of two.
    persistedComposerSnapshot.text = input.value;
    syncSlashStateFromInput(input);
    resetTabCompletion();
    clearVoiceError();
    // rAF-batched: resizeInput forces a synchronous layout (height='auto' then
    // reads scrollHeight), so coalesce it to once per frame rather than once
    // per keystroke/paste-chunk.
    scheduleResize();
  };
  ...
  return (
    <>
              ...
              return (
                <div key={item.id} className="relative group border border-border rounded px-2 py-1.5 bg-background min-w-[190px] max-w-[260px]">
                  <button
                    ...
                    className="absolute -top-1.5 -right-1.5 w-6 h-6 bg-red-600 text-white rounded-full flex items-center justify-center text-[10px] opacity-80 hover:opacity-100 cursor-pointer"
                  >
                    <X size={10} />
                  </button>
      ...
      >
        ...
        {isSlashMenuOpen && (
          <div className="absolute inset-x-0 bottom-full z-20 mb-2 overflow-hidden rounded-2xl border border-border/70 bg-popover/95 shadow-[0_18px_40px_rgba(0,0,0,0.28)]">
        ...
        {/* Documents are extracted to text at the gateway and delivered to the
            agent. Images are refused with the reason (no configured model can
            read them) — never dropped silently. See central_command/api/attachments.py. */}
        <button
          type="button"
          ...
          title={uploadsEnabled
            ? 'Attach a document (PDF, DOCX, XLSX, PPTX, CSV, HTML, text)'
            : 'Attachments are disabled by configuration'}
