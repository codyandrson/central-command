...
interface ConnectDialogProps {
  ...
  /** False when the deployment requires no auth at all (config.auth off) —
   * an empty token is then acceptable even without serverSideAuth. */
  authEnabled?: boolean;
}
...
export function ConnectDialog({
  ...
  const handleConnect = async () => {
    ...
    // An empty token is acceptable either when the server will inject one
    // (serverSideAuth) or when the deployment requires no auth at all
    // (authEnabled false) — the Central Command default with no GATEWAY_TOKEN set.
    const emptyTokenOk = (serverSideAuth || !authEnabled) && isOfficialUrl;
    if (!url.trim() || (!token.trim() && !emptyTokenOk)) return;
    ...
    const effectiveToken = emptyTokenOk ? '' : token.trim();
