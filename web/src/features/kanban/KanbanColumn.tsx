...
export const KanbanColumn = memo(function KanbanColumn({ status, tasks, onCardClick, label }: KanbanColumnProps) {
  const accent = TASK_STATUS_TONE[status] ?? TASK_STATUS_TONE['NEW'];
  ...
  return (
    ...
    >
      <div className="sticky top-0 z-10 flex h-11 items-center justify-between border-b border-border/55 bg-card/78 px-3">
