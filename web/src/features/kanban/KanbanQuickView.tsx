...
/* ── Statuses shown in quick view: the non-terminal half of the lifecycle ── */
const QUICK_STATUSES: TaskStatus[] = ['NEW', 'ASSIGNED', 'IN_PROGRESS', 'REVIEW'];
const MAX_ROWS = 5;

interface KanbanQuickViewProps {
  onOpenBoard: () => void;
  onOpenTask: (task: KanbanTask) => void;
  /** Scope the panel to one agent's tasks — the workspace's selected agent. */
  agentId?: string;
}
...
export function KanbanQuickView({ onOpenBoard, onOpenTask, agentId }: KanbanQuickViewProps) {
  const { tasksByStatus, loading, error } = useKanban(agentId ? `agent:${agentId}` : undefined);
  ...
  const totalActive = sections.reduce((n, s) => n + s.tasks.length, 0);
