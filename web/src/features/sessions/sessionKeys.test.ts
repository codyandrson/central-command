...
describe('isCentralCommandSessionKey', () => {
  it('matches the real lane keys the gateway produces', () => {
    // `nerve_gateway._session_key` => "agent:<agent_id>:<session_id>", and
    // session ids are "sess_" + 12 hex chars.
    expect(isCentralCommandSessionKey('agent:coach:sess_a1b2c3d4e5f6')).toBe(true);
    expect(isCentralCommandSessionKey('agent:jira-expert:sess_0123456789ab')).toBe(true);
  });

  it('does NOT match the synthetic agent root', () => {
    // The whole reason this predicate exists separately: widening
    // `isTopLevelAgentSessionKey` to cover lanes would nest the tree wrongly
    // and break the default-session picker.
    expect(isCentralCommandSessionKey('agent:coach:main')).toBe(false);
    expect(isTopLevelAgentSessionKey('agent:coach:sess_a1b2c3d4e5f6')).toBe(false);
  });

  it('does NOT match vendored Nerve key shapes', () => {
    expect(isCentralCommandSessionKey('agent:main:subagent:abc123')).toBe(false);
    expect(isCentralCommandSessionKey('agent:main:cron:nightly')).toBe(false);
    expect(isCentralCommandSessionKey('discord:sean')).toBe(false);
    expect(isCentralCommandSessionKey('agent:coach:sess_NOTHEX')).toBe(false);
  });
});
