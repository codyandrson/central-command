...
import { getExplicitParentCandidates, getRootAgentSessionKey, getSessionType, isTopLevelAgentSessionKey, resolveParentSessionKey, TERMINAL_SESSION_STATUSES } from './sessionKeys';
...
function buildTreeNodes(
  ...
  const isOpen = (session: Session) =>
    !TERMINAL_SESSION_STATUSES.has((session.status || '').toUpperCase());

  /** Does this agent have work in flight? Its CHILD sessions answer that, never
   *  its own row: an agent root is a synthetic per-roster-agent placeholder the
   *  gateway stamps `IDLE` (`_sessions_list`), so reading the root's status
   *  would call every active agent busy. Direct children only — a live
   *  grandchild under a landed child is not a shape this sidebar produces. */
  const isBusyRoot = (rootKey: string) =>
    (childrenOf.get(rootKey) ?? []).some(isOpen);
  ...
  function buildNodes(parentKey: string | null, depth: number): TreeNode[] {
    ...
    const sorted = [...children].sort((a, b) => {
      ...
      if (parentKey === null && isTopLevelAgentSessionKey(keyA) && isTopLevelAgentSessionKey(keyB)) {
        // Agents with work in flight sort above idle ones. Past a dozen agents
        // an alphabetical roster buries the one that is actually running, and
        // "who has something open" is the question this sidebar is scanned for.
        // Computed from the SESSIONS, not from child count: the History toggle
        // puts closed conversations back in the tree, and a closed session is
        // not work.
        const busyA = isBusyRoot(keyA) ? 0 : 1;
        const busyB = isBusyRoot(keyB) ? 0 : 1;
