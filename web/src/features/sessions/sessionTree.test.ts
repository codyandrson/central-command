...
describe('buildAgentSidebarTree', () => {
  ...
  it('sorts agents with open work above idle ones, then alphabetically', () => {
    // Agent roots are synthetic per-roster rows the gateway stamps IDLE, so
    // "working" has to come from the CHILD sessions — reading the root's own
    // status would put every active agent in the busy bucket.
    const sessions = [
      session('agent:accountability:main', { label: 'Accountability', status: 'IDLE' }),
      session('agent:zeta:main', { label: 'Zeta', status: 'IDLE' }),
      session('agent:zeta:sess_1', {
        label: 'Zeta run', status: 'AWAITING_HUMAN', parentSessionKey: 'agent:zeta:main',
      }),
      session('agent:coach:main', { label: 'Coach', status: 'IDLE' }),
      // A landed child is not work — this agent stays idle.
      session('agent:coach:sess_9', {
        label: 'Coach run', status: 'DONE', parentSessionKey: 'agent:coach:main',
      }),
    ];

    const roots = buildAgentSidebarTree(sessions).map((node) => node.key);
    expect(roots).toEqual([
      'agent:zeta:main',
      'agent:accountability:main',
      'agent:coach:main',
    ]);
  });
});
