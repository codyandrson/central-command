...
export const SessionInfoPanel = memo(function SessionInfoPanel({
  ...
  const ctxTok = session.contextTokens || undefined;
  const pct = tokenPct(totalTok, ctxTok);
  const barCls = ctxTok ? barColor(pct) : 'bg-muted-foreground/20';
  ...
  const status = TERMINAL_SESSION_STATUSES.has(statusUpper)
    ? 'CLOSED'
    : statusUpper === 'STOPPED'
      ? 'STOPPED'
