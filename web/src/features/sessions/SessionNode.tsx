...
import { TERMINAL_SESSION_STATUSES } from './sessionKeys';
...
// Unknown limit: muted, no percent-derived color/glow — never invent a denominator.
const COLORS_UNKNOWN = {
...
interface SessionNodeProps {
  ...
  /** Tier 3: this discussion has gone quiet and is holding a task. */
  stall?: { taskTitle: string; waitingFor: string } | null;
  /** The AGENT opened this lane and is waiting on an answer. */
  asked?: { taskTitle: string | null; waitingFor: string } | null;
...
function arePropsEqual(prev: SessionNodeProps, next: SessionNodeProps): boolean {
  return (
    ...
    prev.stall?.taskTitle === next.stall?.taskTitle &&
    prev.stall?.waitingFor === next.stall?.waitingFor &&
    prev.asked?.taskTitle === next.asked?.taskTitle &&
    prev.asked?.waitingFor === next.asked?.waitingFor &&
...
export const SessionNode = memo(function SessionNode({
  ...
  const max = session.contextTokens;
  const pct = max ? Math.min(100, Math.round((displayTokens / max) * 100)) : 0;
  const colors = !max ? COLORS_UNKNOWN : pct >= 80 ? COLORS_CRITICAL : pct >= 50 ? COLORS_WARNING : COLORS_NORMAL;
  ...
  const handleNewSessionClick = useCallback((e: React.MouseEvent) => {
    e.stopPropagation();
  ...
  // Central Command conversation rows are closable too (close = archived, never
  // deleted) — without this, plain session rows had NO close control at all.
  const isConversation = node.session.mode === 'conversation';
  const hasAbortAction = Boolean(onAbort && running);
  const hasRenameAction = Boolean(canRenameDelete && onStartRename && !isRenaming);
  const hasDeleteAction = Boolean(
    (canRenameDelete || isConversation || Boolean(node.session.execFailure)) && onDelete
  );
  // "+" on conversational agent roots: open an ADDITIONAL session — everything
  // already open stays open (multi-session per agent).
  const hasNewSessionAction = Boolean(
    isRootAgent && node.session.conversational && onNewSession,
  );
  const hasActions = hasAbortAction || hasRenameAction || hasDeleteAction || hasNewSessionAction;
  ...
  const handleNewSessionFromMenu = useCallback((e: React.MouseEvent) => {
    handleNewSessionClick(e);
    setActionsOpen(false);
  }, [handleNewSessionClick]);

  // A terminal session can't be working — its closed state beats any stale
  // granular status, so the currently-viewed row (exempt from the history
  // filter) says CLOSED instead of lying "IDLE".
  const terminalStatus = (session.status || '').toUpperCase();
  const isClosed = TERMINAL_SESSION_STATUSES.has(terminalStatus);
  // A stopped session can't be working either, same reasoning as isClosed —
  // its own state beats a stale granular status, which would otherwise say
  // "IDLE" for a run the operator explicitly paused.
  const isStopped = !isClosed && terminalStatus === 'STOPPED';

  // An exec-failed session is resting OPEN, never terminal — it beats every
  // other badge below CLOSED/STOPPED because the operator must see it and
  // close it by hand; the reason rides in the title for hover.
  const execFailed = !isClosed && !isStopped && Boolean(session.execFailure);

  // A lane RESTING on a human beats every granular frame. `granularStatus` is
  // client-side, fed by push frames, and nothing clears it when a run parks —
  // so the last tool call of a parked run kept pulsing "TOOL: PROPOSE_ACTION"
  // over a session that was doing nothing at all (2026-08-26). The authoritative
  // session status says who is actually holding the turn; only a RUNNING lane
  // gets the granular/active chip below.
  // `running` (busyState / state==='running') still wins: a turn the operator
  // just started is in flight before the next sessions.list refresh says so.
  const awaitingText = running || isClosed || isStopped || execFailed ? ''
    : terminalStatus === 'AWAITING_HUMAN' ? 'AWAITING DECISION'
      : terminalStatus === 'AWAITING_OPERATOR' ? 'YOUR TURN' : '';

  // Compute badge text and classes from granular status or fall back to binary
  const badgeText = isClosed
    ? (terminalStatus === 'DONE' ? 'CLOSED' : terminalStatus)
    : isStopped
      ? 'STOPPED'
      : execFailed
        ? 'EXEC FAILED'
  ...
  const badgeClasses = isClosed
    ? (terminalStatus === 'FAILED' ? 'bg-red/20 text-red' : 'bg-muted-foreground/20 text-muted-foreground')
    : isStopped
      ? 'bg-orange/20 text-orange'
      : execFailed
        ? 'bg-red/20 text-red'
        // Waiting on YOU: steady, never animated — the pulse is what read as
        // "still working" on a lane that had stopped.
        : awaitingText
  ...
  return (
    ...
    >
      ...
      >
        ...
        {/* The agent opened this lane and is waiting. An agent-initiated
            conversation is otherwise indistinguishable from one the operator opened
            himself, which is the difference between "I'll get to it" and
            "someone is standing still because of me". Same inline, shrink-0
            treatment as the stall chip: nothing moves, nothing is squeezed.
            Suppressed when the stall chip is showing — a quiet lane already
            says everything this one would, louder. */}
        {asked && !stall && (
          <span
            data-testid="session-asked-chip"
            title={asked.taskTitle
              ? `${node.session.agentId ?? 'The agent'} asked you — blocks ${asked.taskTitle} · waiting ${asked.waitingFor}`
              : `${node.session.agentId ?? 'The agent'} asked you · waiting ${asked.waitingFor}`}
            className="shrink-0 max-w-[9rem] truncate rounded-sm bg-primary/20 px-1.5 py-0.5 text-[0.6rem] font-bold text-primary"
          >
            asked you · {asked.waitingFor}
          </span>
        )}

        {/* Tier-3 stall: name the task this quiet discussion is holding. Sits
            inside the existing row: same height, same order, and shrink-0 like
            every other trailing element, so it cannot squeeze the session name
            the operator is reading. */}
        {stall && (
          <span
            data-testid="session-stall-chip"
            title={`Blocked: ${stall.taskTitle} — this discussion has been quiet for ${stall.waitingFor}`}
            className="shrink-0 max-w-[9rem] truncate rounded-sm bg-orange/20 px-1.5 py-0.5 text-[0.6rem] font-bold text-orange"
          >
            blocks {stall.taskTitle} · waiting {stall.waitingFor}
          </span>
        )}
        ...
        <span
          title={execFailed ? session.execFailure ?? undefined : undefined}
      ...
      {!compact && hasDeleteAction && !isClosed && (
        <button
          ...
          title="Close session (kept in history)"
          className="shrink-0 mr-1 bg-transparent border-0 text-muted-foreground hover:text-red cursor-pointer text-[0.667rem] w-5 h-5 flex items-center justify-center"
        >
          ✕
        </button>
      )}
      ...
      {compact ? (
              ...
              <>
                {hasNewSessionAction && (
                  <button
                    ...
                    title="Open a new session (existing sessions stay open)"
                ...
                {hasAbortAction && (
                  <button
                    ...
                    title="Abort session"
                    className="bg-card border border-border/60 text-muted-foreground hover:text-red hover:border-red/40 cursor-pointer text-[0.667rem] w-6 h-6 flex items-center justify-center"
                  >
                    ⏹
                  </button>
                )}
                {hasRenameAction && (
                  <button
                    ...
                    className="bg-card border border-border/60 text-muted-foreground hover:text-foreground hover:border-muted-foreground cursor-pointer text-[0.667rem] w-6 h-6 flex items-center justify-center"
                  >
                    <PenLine size={10} />
                  </button>
                )}
        ...
        <div className="absolute right-1 top-1/2 -translate-y-1/2 opacity-0 group-hover:opacity-100 flex items-center gap-0.5 transition-opacity z-10">
          ...
          {(canRenameDelete || isConversation) && (
            <>
              {hasRenameAction && (
                <button
                  type="button"
