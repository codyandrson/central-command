...
// Central Command terminal session statuses — these rows are history (archived,
// never deleted). SessionList hides them behind its History toggle; the status
// bar excludes them from the open-session count.
export const TERMINAL_SESSION_STATUSES = new Set(['DONE', 'FAILED', 'CANCELLED']);

const ROOT_AGENT_RE = /^agent:([^:]+):main$/;
/**
 * A REAL Central Command run/conversation lane: `agent:<id>:sess_<hex>`, the keys
 * `nerve_gateway._session_key` actually produces.
 *
 * Deliberately separate from `isTopLevelAgentSessionKey`, which matches only
 * the synthetic `agent:<id>:main` roots and is load-bearing for the session
 * tree, the default-session picker and the model fallback in GatewayContext.
 * Widening THAT predicate would nest every lane wrongly; this one exists so
 * per-session behaviour (unread, chips) can key off a lane without touching it.
 */
const CC_SESSION_RE = /^agent:[^:]+:sess_[a-f0-9]+$/;
...
export function isCentralCommandSessionKey(sessionKey: string): boolean {
  return CC_SESSION_RE.test(sessionKey);
}
