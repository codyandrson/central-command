        ...
        <div className="flex items-start gap-2.5">
          <div className="min-w-0 flex-1 space-y-1.5">
            <div className="flex flex-wrap items-start justify-between gap-2.5">
              <div className="min-w-0 flex-1 basis-40 flex items-start gap-2">
              <button
...
export function CronsTab() {
  ...
  const handleEngineToggle = useCallback(async () => {
    ...
    setEngineBusy(true);
    try {
      await setEngineRunning(!engine.running);
    } finally {
      setEngineBusy(false);
    }
  }, [engine, setEngineRunning]);
    ...
    <div className="h-full flex flex-col min-h-0">
      <div className="flex-1 overflow-y-auto">
        <div className="space-y-2 px-2.5 py-2.5">
          ...
          {/* Heartbeat engine state — an enabled schedule is dormant while the
              engine is stopped, and that must never read as live. */}
          {engine && (
            <div
              className="cockpit-note flex flex-wrap items-center justify-between gap-2 px-3 py-2 text-[0.733rem]"
              data-tone={engine.running ? undefined : (toolbarSummary.enabledCount > 0 ? 'danger' : undefined)}
            >
              <div className="min-w-0 space-y-0.5">
                <div className="flex items-center gap-1.5">
                  <Circle
                    size={8}
                    fill={engine.running ? 'currentColor' : 'none'}
                    className={engine.running ? 'text-green' : 'text-muted-foreground'}
                  />
                  <span className="font-medium">
                    {engine.running
                      ? 'Heartbeat engine running'
                      : 'Heartbeat engine stopped'}
                  </span>
                </div>
                <div className="text-muted-foreground">
                  {engine.running
                    ? `Enabled schedules fire on cadence${engine.tickSeconds ? ` (checked every ${engine.tickSeconds}s)` : ''}.`
                    : toolbarSummary.enabledCount > 0
                      ? `${toolbarSummary.enabledCount} enabled schedule${toolbarSummary.enabledCount === 1 ? ' is' : 's are'} dormant — nothing fires until the engine starts${engine.reason ? ` (${engine.reason})` : ''}.`
                      : 'No schedules are enabled; nothing is waiting on it.'}
                </div>
              </div>
              <button
                type="button"
                ...
                disabled={engineBusy}
                className="shell-chip min-h-8 shrink-0 rounded-lg px-2.5 text-[0.7rem] font-medium disabled:opacity-60"
                title="Applies until the next server restart — the durable switch is CC_HEARTBEAT_ENABLED"
                aria-label={engine.running ? 'Stop heartbeat engine' : 'Start heartbeat engine'}
              >
                {engineBusy
