...
export interface CronJob {
  ...
  model?: string;
  thinking?: string;
  agentId?: string;       // Central Command: the assigned agent for task.create schedules
  actionKind?: string;    // Central Command: the heartbeat action behind this schedule
...
/** A heartbeat action registry entry (served by the Central Command API). */
export interface CronActionSpec {
  description: string;
...
export function normalizeCronJob(j: Record<string, unknown>): CronJob {
  ...
  return {
    ...
    actionKind: (j.actionKind as string | undefined) || undefined,
    actionParams: (j.actionParams as Record<string, unknown> | undefined) || undefined,
    sessionTarget:
...
export function useCrons() {
  ...
  const [engine, setEngine] = useState<CronEngine | null>(null);
  const [actions, setActions] = useState<Record<string, CronActionSpec>>({});
  ...
  const fetchJobs = useCallback(async () => {
    ...
    try {
      ...
      }>('/api/crons', undefined, 'Failed to fetch crons');
      ...
      setEngine(data.result?.engine ?? null);
  ...
  const toggleJob = useCallback(async (id: string, enabled: boolean) => {
    try {
      await apiFetch(`/api/crons/${encodeURIComponent(id)}/toggle`, {
        method: 'POST',
  ...
  const runJob = useCallback(async (id: string) => {
    try {
      await apiFetch(`/api/crons/${encodeURIComponent(id)}/run`, { method: 'POST' }, 'Failed to run');
  ...
  const fetchRuns = useCallback(async (id: string): Promise<CronRun[]> => {
    try {
      ...
      }>(`/api/crons/${encodeURIComponent(id)}/runs`, undefined, 'Failed to fetch runs');
  ...
  const deleteJob = useCallback(async (id: string) => {
    try {
      await apiFetch(`/api/crons/${encodeURIComponent(id)}`, { method: 'DELETE' }, 'Failed to delete');
  ...
  const setEngineRunning = useCallback(async (running: boolean) => {
    try {
      const data = await apiFetch<{ result?: CronEngine }>('/api/crons/engine', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ running }),
      }, 'Failed to change engine state');
      setEngine(data.result ?? null);
      return true;
    } catch (err) {
      setErrorState(err);
      return false;
  ...
  return { jobs, engine, actions, isLoading, error, activeCount, fetchJobs, toggleJob, runJob, fetchRuns, addJob, updateJob, deleteJob, setEngineRunning };
}
