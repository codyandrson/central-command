...
export function useChatMessages({ rpc, currentSessionRef }: UseChatMessagesDeps) {
  ...
  // Central Command: whether this session accepts input, from the history payload.
  const [composer, setComposer] = useState<ChatComposerState | null>(null);
  // Central Command: the email this lane was spawned to handle, from the same payload.
  ...
  const loadHistory = useCallback(async (session?: string) => {
    ...
    try {
      const result = await loadChatHistory({
        ...
        sessionKey: sk,
        limit: 500,
        // Late arrivals must not govern a session the operator has since left,
        // or a read-only run's notice would sit over an open conversation.
        onComposer: (state, forKey) => {
          if (forKey === (currentSessionRef.current || sk)) setComposer(state ?? null);
        },
        onSourceEmail: (email, forKey) => {
          if (forKey === (currentSessionRef.current || sk)) setSourceEmail(email ?? null);
        },
      });
      ...
      setHasMore(false);
      // Unknown, not "read-only": a failed read must not leave the previous
      // session's notice standing over this one. The failure itself is shown.
      setComposer(null);
      setSourceEmail(null);
