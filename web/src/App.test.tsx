...
vi.mock('@/components/Splitter', () => ({
  SplitGroup: ({ children }: { children: ReactNode }) => <div>{children}</div>,
  SplitSeparator: () => <div role="separator" />,
...
describe('App save toast workspace scoping', () => {
  ...
  // The next three tests exercise the file tree's add-to-chat, deferred while
  // App.tsx's FILE_BROWSER_ENABLED is hardcoded false (2026-07-23) — the panel
  // never renders. Unskip them when that flag flips.
  it.skip('passes the active workspace agent through add-to-chat requests from the file tree', async () => {
  ...
  it.skip('does not expose add-to-chat from the file tree when file references are disabled', async () => {
  ...
  it.skip('retries upload-config after a transient failure before hiding add-to-chat', async () => {
    vi.useFakeTimers();
