...
import { CcLiveBridge } from '@/components/CcLiveBridge';
...
import { buildAgentRootSessionKey, getSessionDisplayLabel, isTopLevelAgentSessionKey, TERMINAL_SESSION_STATUSES } from '@/features/sessions/sessionKeys';
import { useStalledDiscussions } from '@/features/sessions/useStalledDiscussions';
...
export default function App({ onLogout }: AppProps) {
  ...
  // producing real file deliverables. See STATUS.md "file browser".
  const FILE_BROWSER_ENABLED = false;
  ...
  // Per-tab "awaits you" counts for the top-bar notification badges.
  const attention = useAttention();

  // Tier-3 clarification discussions the stall sweep has flagged — the
  // sessions panel marks those rows with the task they are holding.
  const stalledDiscussions = useStalledDiscussions();
  ...
  // A blocked run, the ask holding it and the task it belongs to are three
  // screens; these carry the operator between them with the item already
  // selected, instead of making them hunt for what they were just shown.
  const [pendingDecisionItemId, setPendingDecisionItemId] = useState<string | null>(null);
  ...
  // "Currently viewing a session" = chat view active AND it's the selected
  // one — SessionContext only knows the selected half, so tell it the other.
  useEffect(() => {
    setViewingChat(viewMode === 'chat');
  }, [viewMode, setViewingChat]);
  const openTaskInBoard = useCallback((taskId: string) => {
    setPendingTaskId(taskId);
    setViewMode('kanban');
  }, [setViewMode]);
  /** Go to the Decisions Inbox — with an item already selected when we know
   *  which one the operator was just looking at. */
  const openDecisionsFor = useCallback((itemId?: string) => {
    setPendingDecisionItemId(itemId ?? null);
    setViewMode('inbox');
  }, [setViewMode]);
  ...
  const contextTokens = currentSessionData ? (currentSessionData.totalTokens ?? 0) : undefined;
  // 0 or missing means the gateway didn't report a limit — unknown, never assumed.
  const contextLimit = currentSessionData?.contextTokens || null;
      ...
      chatPanel={
        <PanelErrorBoundary name="Chat">
          <ChatPanel
            ...
            loadMore={loadMore}
            hasMore={hasMore}
            onToggleFileBrowser={FILE_BROWSER_ENABLED ? (isCompactLayout ? handleToggleFileBrowser : fileBrowserCollapsed ? handleToggleFileBrowser : undefined) : undefined}
            ...
            onOpenDecisions={openDecisionsFor}
            // The board is hideable; without it there is nowhere to go, and a
  ...
  const renderRightPanels = (onSelect: (key: string) => Promise<void> | void) => (
    <Suspense fallback={<div className="flex-1 flex items-center justify-center text-muted-foreground text-xs bg-background">Loading…</div>}>
      {/* Sessions + Memory stacked vertically, boundary draggable */}
      <SplitGroup id="cc-sessions-workspace" orientation="vertical" className="flex-1 min-h-0">
        <Panel id="sessions" minSize={160} className="shell-panel flex flex-col min-h-0 rounded-[28px]" style={{ overflow: 'hidden' }}>
          <PanelErrorBoundary name="Sessions">
            <SessionList
              sessions={sessions}
              currentSession={currentSession}
              isViewingChat={viewMode === 'chat'}
              busyState={busyState}
              ...
              agentName={agentName}
              stalledDiscussions={stalledDiscussions}
            />
          </PanelErrorBoundary>
        </Panel>
        <SplitSeparator vertical />
        <Panel id="workspace" className="shell-panel flex flex-col min-h-0 rounded-[28px]" style={{ overflow: 'hidden' }}>
  ...
  const showCompactFileBrowser = FILE_BROWSER_ENABLED && isCompactLayout && viewMode === 'chat' && !fileBrowserCollapsed;
    ...
    <div className="scan-lines relative h-screen flex flex-col overflow-hidden" data-booted={booted}>
      ...
      {executorMode === 'dry_run' && (
        <div
          ...
          className="flex shrink-0 items-center justify-center gap-2 bg-orange/20 px-4 py-1.5 text-[0.733rem] font-semibold text-orange"
        >
          <AlertTriangle size={13} aria-hidden="true" />
          DRY RUN — approvals are simulated, nothing is written for real
        </div>
      )}
      ...
      {/* Interrupt-tier notifications. Fixed-position (never in the flow), and
          every toast lands on the record it is about. */}
      <ToastHost
        onOpenDecision={openDecisionsFor}
        onOpenSession={(key) => { setViewMode('chat'); handleSessionChange(key); }}
      />
      ...
      <div className="flex-1 flex gap-3 overflow-hidden min-h-0 px-2 pt-1.5 pb-2 sm:px-4 sm:pt-2 sm:pb-2">
        {/* File tree — desktop inline, mobile drawer (deferred: flag above) */}
        {FILE_BROWSER_ENABLED && !isCompactLayout && (
        ...
        {showCompactFileBrowser && (
          <>
            <button
              type="button"
              className="fixed inset-0 z-30 hidden bg-black/48 max-[900px]:block"
          ...
          <div className="shell-panel boot-panel flex-1 flex flex-col min-w-0 min-h-0 overflow-hidden rounded-[28px]">
            <Suspense fallback={<div className="flex-1 flex items-center justify-center text-muted-foreground text-xs bg-background">Loading…</div>}>
              <KanbanPanel
                ...
                onOpenSession={(sessionKey) => { setViewMode('chat'); handleSessionChange(sessionKey); }}
              />
            </Suspense>
          </div>
        )}
        {viewMode === 'inbox' && (
          <div className="shell-panel boot-panel flex-1 flex flex-col min-w-0 min-h-0 overflow-hidden rounded-[28px]">
            <Suspense fallback={<div className="flex-1 flex items-center justify-center text-muted-foreground text-xs bg-background">Loading…</div>}>
              <DecisionsView
              initialItemId={pendingDecisionItemId}
              onInitialItemConsumed={() => setPendingDecisionItemId(null)}
              onOpenTask={kanbanVisible ? openTaskInBoard : undefined}
              onOpenSession={(sessionKey) => { setViewMode('chat'); handleSessionChange(sessionKey); }}
            />
            </Suspense>
        ...
        {viewMode === 'agents' && (
          <div className="shell-panel boot-panel flex-1 flex flex-col min-w-0 min-h-0 overflow-hidden rounded-[28px]">
            <Suspense fallback={<div className="flex-1 flex items-center justify-center text-muted-foreground text-xs bg-background">Loading…</div>}>
              <AgentsView
                onOpenDecisions={() => setViewMode('inbox')}
                onOpenTask={kanbanVisible ? openTaskInBoard : undefined}
              />
            </Suspense>
          </div>
        ...
        {viewMode === 'graph-verify' && (
          <div className="shell-panel boot-panel flex-1 flex flex-col min-w-0 min-h-0 overflow-hidden rounded-[28px]">
            <Suspense fallback={<div className="flex-1 flex items-center justify-center text-muted-foreground text-xs bg-background">Loading…</div>}>
              <GraphVerificationsView />
            </Suspense>
          </div>
        )}
        {viewMode === 'activity' && (
            ...
            <Suspense fallback={<div className="flex-1 flex items-center justify-center text-muted-foreground text-xs bg-background">Loading…</div>}>
              <ActivityView
                onOpenSession={(sessionId, agentId) => {
                  setViewMode('chat');
                  handleSessionChange(`agent:${agentId}:${sessionId}`);
                }}
              />
            </Suspense>
          </div>
        ...
        {isCompactLayout ? (
          ...
          <div style={{ display: viewMode !== 'chat' ? 'none' : 'contents' }}>
            <SplitGroup id="cc-chat-sessions" className="flex-1 min-h-0">
              <Panel
                id="chat"
                defaultSize="55"
                minSize="30"
                maxSize="85"
                className="shell-panel boot-panel rounded-[28px]"
                style={{ overflow: 'hidden' }}
              >
                {chatContent}
              </Panel>
              <SplitSeparator />
              {/* Collapsing the file browser widens this group. The right pane
                  used to be pinned to its pixel width so the CHAT absorbed the
                  freed space; `preserve-pixel-size` is that same rule, declared. */}
              <Panel
                id="sessions"
                groupResizeBehavior={fileBrowserCollapsed ? 'preserve-pixel-size' : 'preserve-relative-size'}
                className="boot-panel flex flex-col"
                style={{ overflow: 'hidden' }}
              >
      ...
      <div className="boot-panel" style={{ transitionDelay: '200ms' }}>
        <StatusBar
          ...
          sessionCount={sessions.filter(s =>
            !isTopLevelAgentSessionKey(getSessionKey(s)) &&
      ...
      {/* New Session Confirmation — closes the current session (kept in
          history; sessions are never destroyed) so the next message starts
          a fresh one. */}
      <ConfirmDialog
        open={showResetConfirm}
        title="Start a New Session"
        message="This closes the current session — it stays in history with its full transcript. Your next message starts a fresh session."
