...
export function describeToolUse(toolName: string, input: Record<string, unknown>): string | null {
  ...
  switch (tn) {
    ...
    default: {
      // Central Command's tools (jira_search, graph_search, propose_action, …) are
      // none of the vendored cases above, so the live activity row said only
      // "using jira_search" — the operator's question is WHICH search. Show the
      // first identifying string argument if the call has one.
      const hint = ['query', 'jql', 'q', 'url', 'issue_key', 'capability', 'name', 'path']
        .map(k => p[k])
        .find(v => typeof v === 'string' && v.trim());
      return hint ? `${toolName}: ${String(hint).trim().slice(0, 80)}` : 'using ' + toolName;
    }
  }
}
