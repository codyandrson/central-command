...
import { getSessionKey, type GatewayEvent, type ChatComposerState, type SourceEmail } from '@/types';
import {
...
interface ChatContextValue {
  ...
  hasMore: boolean;
  /** Central Command: whether this session accepts operator input, and if not, why
   *  (served with the history so the composer and the send cannot disagree). */
...
export function ChatProvider({ children }: { children: ReactNode }) {
  ...
  useEffect(() => {
    return subscribe((msg: GatewayEvent) => {
      ...
      // Central Command turn failures — render them; a failed turn that only
      // logs to console reads as "still thinking" forever.
      if (msg.event === 'cc.chat.error') {
        const p = msg.payload as { sessionKey?: string; message?: string } | undefined;
        if (p?.sessionKey && p.sessionKey === currentSessionRef.current) {
          const bubble: ChatMsg = {
            msgId: generateMsgId(),
            role: 'system',
            html: `⚠️ Turn failed: ${p.message || 'unknown error'}`,
            rawText: '',
            timestamp: new Date(),
          };
          msgHook.setAllMessages(prev => [...prev, bubble]);
          msgHook.setMessages((prev: ChatMsg[]) => [...prev, bubble]);
          setIsGenerating(false);
  ...
  const handleAbort = useCallback(async () => {
    try {
      ...
      // Say so — a Stop click that silently does nothing reads as a hang.
      const detail = err instanceof Error ? err.message : String(err);
      const bubble: ChatMsg = {
        msgId: generateMsgId(),
        role: 'system',
        html: `⚠️ Stop is not available: ${detail}. Central Command turns run to completion (usually well under a minute) — the reply lands here when the turn finishes.`,
        rawText: '',
        timestamp: new Date(),
      };
      msgHook.setAllMessages(prev => [...prev, bubble]);
  ...
  /** Resume a session the operator stopped. `sessionId` is the raw Central Command
   * session id (composer.session.id) — `sessions.resume` resolves it straight
   * from that when no cockpit sessionKey applies. On success, reload the
   * current session's history so the composer's "stopped" notice clears from
   * the server's own account, not a client-invented state flip. */
  const handleResume = useCallback(async (sessionId: string) => {
    await rpc('sessions.resume', { sessionId });
  ...
  const confirmReset = useCallback(async () => {
    ...
    try {
      // Central Command semantics: this CLOSES the current conversation (kept in
      // history — sessions are never destroyed); the next message starts a
      // fresh one. The pane empties because a new session begins, not because
      // anything was cleared.
      const currentKey = currentSessionRef.current;
      const result = await rpc('sessions.reset', { key: currentKey }) as
        { session_id?: string | null; status?: string };
      const closed = Boolean(result?.session_id);
      // Move the selection off the closed session's key to the agent's root
      // (the fresh lane) — a selection left on the closed row keeps it
      // exempt from the panel's history filter forever.
      const [prefix, agentId] = currentKey.split(':');
      if (closed && prefix === 'agent' && agentId && currentKey !== `agent:${agentId}:main`) {
      ...
      const msg: ChatMsg = {
        ...
        role: 'system',
        html: closed
          ? '⚙️ Session closed and kept in history. Your next message starts a fresh session.'
          : '⚙️ No open session — your next message starts a fresh one.',
        rawText: '',
        timestamp: new Date(),
      };
      ...
      const msg: ChatMsg = {
        msgId: generateMsgId(),
        role: 'system',
        html: `⚙️ Could not close the session: ${errMsg}`,
        rawText: '',
        timestamp: new Date(),
      };
  ...
  const value = useMemo<ChatContextValue>(() => ({
    ...
    composer: msgHook.composer,
    sourceEmail: msgHook.sourceEmail,
    showResetConfirm,
    confirmReset,
    cancelReset,
  }), [
