...
// No stored gateway config: there is exactly one gateway, the server knows it,
// and a persisted `oc-config` could only ever go stale. (Removed 2026-08-26.)

export function GatewayProvider({ children }: { children: ReactNode }) {
  const { connectionState, connect, disconnect, rpc, onEvent, connectError, reconnectAttempt } = useWebSocket();
