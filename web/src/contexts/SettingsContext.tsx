...
import { TOAST_MODE_KEY } from '@/features/notifications/seen';

export type STTProvider = 'local' | 'openai';
export type STTInputMode = 'browser' | 'local' | 'hybrid';

/** Which toasts the operator wants. `errors` keeps the failure toasts — those
 *  are the ones with no badge or inbox row of their own — and drops the
 *  awaiting-you ones the Decisions badge already counts. */
export type ToastMode = 'all' | 'errors' | 'off';
...
/** Same key the notification layer reads — see features/notifications/seen.ts. */
const TOAST_MODE_STORAGE_KEY = TOAST_MODE_KEY;
...
export function SettingsProvider({ children }: { children: ReactNode }) {
  ...
  const [toastMode, setToastModeState] = useState<ToastMode>(() => {
  ...
  const setToastMode = useCallback((mode: ToastMode) => {
    setToastModeState(mode);
    localStorage.setItem(TOAST_MODE_STORAGE_KEY, mode);
  }, []);
