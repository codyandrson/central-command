...
that would not finish. Verified independently rather than taken at face value,
they came apart into five separate causes — one of them ours, three of them
host prerequisites nothing checked, and one a message that named a component
instead of a command. The site was installing v2.41.0, where the answer file
was still `deploy/single/.env`, so some of what they hit is already gone; what
follows is what survived verification.

The one that was ours: **`requirements.lock` could not install on Windows at
all.** It was a `pip freeze` of the Linux venv with no platform markers, so with
`CC_AIRGAP=1` (install from the lock, resolve nothing) a Windows host was told
to install uvloop, which has no Windows build, and onnxruntime 1.29.0, which
magika 0.6.3 caps at 1.20.1 on win32 — and `uv pip install` failed before it
reached anything else. It is now a UNIVERSAL lock, regenerated by
`scripts/regen_lock.sh` with `uv pip compile --universal`, constrained to the
existing pins so that **no Linux pin moved** — the same versions this release
was tested with, plus the environment markers that make the file resolvable on
another platform:

- `uvloop` carries its `sys_platform != 'win32'` exclusion (it has no Windows
  build, and never had).
- `onnxruntime` is FORKED by marker — 1.29.0 off win32, 1.20.1 on it. Not a
  preference: markitdown 0.1.x pins magika 0.6.x, which caps onnxruntime on
  win32 at 1.20.1, and a universal resolve has to satisfy both platforms at
  once.
- The Windows-only dependencies that appear in a universal resolve (colorama,
  pywin32, tzdata and friends) are marker-guarded, so a Linux install
  resolves to exactly what it did before.
- `tests/test_requirements_lock.py` is the guard: uvloop must carry its win32
  exclusion, onnxruntime must be forked, and a duplicate pin without a marker
  is a corrupted regeneration — so a Linux-only lock fails the suite rather
  than the operator's install.

The other four were the host's, and `check` now says so before anything is
installed:

- **podman-compose 1.5.0 is too old, and `check` called it a PASS.** The host
  section only ever asked whether a provider ANSWERED. Two things this profile
  depends on first shipped in podman-compose 1.6.0 (2026-06-03): `up --wait`,
  which is how the deploy phases wait on `compose.yaml`'s healthchecks, and the
  config-hash change that made a second `up -d` idempotent — below it, every
  re-run of the install dies with `container name ... is already in use`. There
  is now a `compose-version` line next to `compose-provider`: a FAIL naming
  both the reason and the fix (`uv tool install podman-compose==1.6.0`, or
  `pip install podman-compose==1.6.0` from the site's PyPI mirror), a PASS with
  the version, and `docker compose` explicitly carrying no floor. The parse
  reads the PRODUCT line, because `podman compose version` prints the
  external-provider banner first and `podman version` second.
- **The host had only Python 3.14, and a missing CPython 3.12 was a WARN.**
  The venv is built with `uv venv --python 3.12`, which downloads an
  interpreter from python-build-standalone (github.com) when the host has
  none — impossible in an air gap. With `CC_AIRGAP=1`, no 3.12 and
  `CC_PYTHON_MIRROR` unset, that line is a FAIL now, naming both ways out
  (install CPython 3.12 on the host, or set the mirror).
- **uv does not read `pip.ini`.** The site had its mirror configured there and
  left `CC_PYPI_INDEX_URL` blank, so every uv resolve went to pypi.org and hung
  against a network that cannot reach it. `CC_PYPI_INDEX_URL` is the seam —
  it is fanned out to `UV_DEFAULT_INDEX` as well as `PIP_INDEX_URL` — and
  `check`'s index section already WARNs when it is blank under `CC_AIRGAP=1`;
  the site never ran `check`, because v2.41.0 had none.
- **`uv run` as the Python fallback SYNCED THE PROJECT.** With no `python3`
  on PATH (Windows), the scripts fall back to `uv run --python 3.12 python`
  — and `uv run` inside the checkout discovers `pyproject.toml`, resolves the
  whole project for every platform (which a mirror holding only Windows
  wheels cannot satisfy) and writes `uv.lock` into the tree. Found on the
  testbed the moment the lock became installable: every `$PY` call in
  `check`'s llm section died on uvloop and the endpoint "listed 0 models".
  The fallback now carries `--no-project`; a test walks the four scripts.
- **The resolver's dry run timed out at 120 seconds.** With the
  `CC_REGISTRY_*` seams unset, every registry request is a TCP connect to a
  public host that will never answer, and each burned the full `--max-time 30`
  before the next. Every curl in `resolve-images.sh`'s `reg_req` now carries
  `--connect-timeout 5`: a connect that has not landed in five seconds is not
  going to, and the run reports instead of expiring.

Two smaller things from the same notes:

- **`update.sh`'s closing line said "start your uvicorn API"** and the
  operator did not know that `./setup.sh boot` is what does it. It now names
  the command: "update applied — start the API with `./setup.sh boot` (or your
  own uvicorn + sandbox runner), then confirm with: ./setup.sh status".
- **The site's checkout path contains a space** (`.../Working Folder/...`).
  Every path expansion in `deploy/single/*.sh` and `deploy/env-lib.sh` was
  audited for it and the scripts were already quoted throughout; `check
  --list`, `preflight` and the resolver's self-test were re-run from a path
  with a space to confirm.

`deploy/AIRGAP.md` and `deploy/single/README.md` now list both host
prerequisites where the tools are listed, and
`tests/test_single_compose_floor.py` pins the floor decision — including the
real three-line `podman compose version` output, banner and all.

## 2026-09-25 — v2.46.0: an update should not re-copy 47,000 files that did not change

The last four findings of the Windows testbed run, each with a measured cause:

- **`update.sh import` took over an hour on NTFS** for a tree `git clone`
  wrote in four minutes. `docs/vendor/` is 47,225 of the repo's 48,420 files
  (553 MB) and it MUST travel in every release zip — it is the air-gapped
  machine's only offline reference — yet almost no release changes it, and
  every import unpacked it, `git rm`'d it, tar-copied it and re-added it.
  `docs/vendor/MANIFEST` is now a committed fingerprint (sha256 over the
  index's blob hashes for that tree, mode-free, milliseconds to compute),
  regenerated by the two vendor fetch scripts through
  `scripts/vendor_manifest.sh` and guarded by a test that fails the suite
  when it is stale. The importer reads the zip's manifest first: equal to the
  deployed one → the whole directory is excluded from unzip, from the
  wipe, from the copy and from the add (`PASS vendor-docs: unchanged … 47225
  files skipped`); different or missing → the full path as before. A zip
  built from a tree with a stale manifest cannot be released, so equal
  manifests mean identical trees.
- **`unzip` reported `symlink error` and exited 0**, so an imported tree
  could silently lose a link. Its stderr is captured; any message or non-zero
  exit is `FAIL unzip` naming it, and the import stops.
- **`check`'s memory probe read the HOST's RAM on a machine-backed podman.**
  It now reads the machine's allocation (`podman machine inspect
  --format '{{.Resources.Memory}}'`, MiB) and names the command that raises
  it; bare Linux keeps `/proc/meminfo`.
- **The test suite hung without limit when the graph's embedder did not
  answer.** The fixtures deliberately let live graph READS run, each of which
  embeds through LiteLLM to an external backend; with that backend off they
  did not fail, they hung — the Windows testbed's "11-hour suite". The nine
  tests that embed carry `@pytest.mark.graph_live`; a session preflight
  probes the read path once with a 10-second budget and SKIPS them with a
  reason naming the cause when it does not answer; and `timeout = 120` is
  now pytest configuration, not a flag anyone has to remember. A `graph_live`
  skip in the gate's summary means the backend was down, not that those
  tests passed.

Owed to the next Windows testbed run: the import fast path and pytest-timeout
under Git Bash, and the machine-memory probe against a real machine.
...
loaded into every agent session — a Graphiti invalidation rule charged to a
session fixing a cron dialog. Nothing was reworded; the file was split by
where each rule applies.

- **`AGENTS.md` is the always-loaded file** (about 2,050 words): a small
  managed block (orientation, policy, where things are, running and
  verifying), the Architecture section, and the 22 governance bite marks a
  session can break from anywhere — proposals, the approval gate, charters,
  the event log, the schema, the sandbox. `CLAUDE.md` is now `@AGENTS.md`,
  because Claude Code reads an `AGENTS.md` on its own only when no
  `CLAUDE.md` exists.
- **Subtree bite marks moved VERBATIM to `.claude/rules/`** — `graph`,
  `deploy-k3s`, `deploy-single`, `models`, `cockpit`, `integrations`,
  `runtime-resilience`, `tests` — each with `paths:` frontmatter, so it loads
  when an agent reads a matching file: before the edit it guards, and never
  in a session that does not go there. New bite marks go in the matching
  rules file, or outside the block markers in `AGENTS.md`; the block itself
  is regenerated by `bmad-project-context`.
- **Two governance rules get the guard they never had**
  (`tests/test_agent_construction_guards.py`): no `Agent(...,
  instructions=...)` — a resume would run under today's charter, not the one
  it proposed under — and no `approval_required`, which would run the
  approved write inside the runtime tier. Source walks, the same shape as
  `test_proposal_created.py`. There is no CI, so the rules keep their lines.
- The stored "Repo layout" tree and the feature inventory are gone (an agent
  reads both more accurately first-hand); the policy facts the tree carried —
  never edit `docs/vendor/`, `servers/` arrives only by approved sync — moved
  into the block. `tests/test_setup_phase_docs.py` now pins the phase list in
  `.claude/rules/deploy-single.md`, and `deploy/k3s/README.md` no longer
  points at a `CLAUDE.md` section that left the public tree in v2.17.2.
...
row (read-aloud, COPY) was hidden below the `sm` breakpoint and
hover-revealed above it — on a phone it did not render at all. It is now a
static row under each message on narrow screens and fades in on hover only
where a pointer can hover (`@media (hover: hover)`), so tablets keep it
visible too. The composer gains a mic button beside Send — press to record,
press again to send — the touch equivalent of double Left Shift, which is
unchanged. Also: `deploy/discover.sh` maps its huggingface.co probe onto the
`CC_HF_ENDPOINT` seam, so a mirror named in `discovery.conf` is probed and
the report emits the `.env` line for the speech engine's model downloads.
Cockpit rebuild only; no manifest change.
